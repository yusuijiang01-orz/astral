...
export async function createRendererQA(pipeline, assets) {
  ...
  const api = {
    ...
    stats() {
      ...
      return {context:!!gl, renderer: ext?gl.getParameter(ext.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER), vendor:gl.getParameter(gl.VENDOR), glError:gl.getError(), programs:pipeline.renderer.info.programs.map(p=>({name:p.name,runnable:p.diagnostics?.runnable ?? true})), calls:pipeline.renderer.info.render.calls, triangles:pipeline.renderer.info.render.triangles, bones:instances[0].root.getObjectByName('QA_Skinned').skeleton.bones.length, materials:[...pipeline.materials].map(m=>m.name), markerPixels:markers.map(m=>{const v=m.position.clone().project(pipeline.camera);return {x:(v.x+1)*innerWidth/2,y:(1-v.y)*innerHeight/2};}), realDevice:'REAL_DEVICE_NOT_VERIFIED'};
    }
  };
